...
def komoditas_ekspor(request):
    ...
    daftar_komoditas_ekspor = Eksportir.objects.all().order_by("profil_company")
...
def kelola_komoditas(request):
    ...
    return render(request, "auth_custom/kelola_komoditas.html", context)
...
@staff_member_required(login_url='login')
def kelola_eksportir(request):
    if request.method == "POST":
        profil_company = request.POST.get("profil_company")
        komoditas = request.POST.get("komoditas")
        satuan = request.POST.get("satuan", "kg")
        tujuan_ekspor = request.POST.get("tujuan_ekspor")
        volume_input = request.POST.get("volume")
        
        volume = 0
        if volume_input:
            val_clean = "".join(filter(str.isdigit, volume_input))
            if val_clean:
                volume = float(val_clean)
                
        if profil_company and komoditas:
            Eksportir.objects.create(
                profil_company=profil_company.strip(),
                komoditas=komoditas.strip(),
                satuan=satuan.strip(),
                tujuan_ekspor=tujuan_ekspor.strip() if tujuan_ekspor else "",
                volume=volume
            )
            messages.success(request, f'Data eksportir "{profil_company}" berhasil ditambahkan!')
        else:
            messages.error(request, "Profil Company dan Komoditas wajib diisi!")
            
        return redirect("kelola_eksportir")
        
    daftar_eksportir = Eksportir.objects.all().order_by("profil_company")
    context = {
        "daftar_eksportir": daftar_eksportir,
    }
    return render(request, "auth_custom/kelola_eksportir.html", context)

@staff_member_required(login_url='login')
def edit_eksportir(request, id):
    eks = get_object_or_404(Eksportir, id=id)
    if request.method == "POST":
        eks.profil_company = request.POST.get("profil_company")
        eks.komoditas = request.POST.get("komoditas")
        eks.satuan = request.POST.get("satuan", "kg")
        eks.tujuan_ekspor = request.POST.get("tujuan_ekspor")
        volume_input = request.POST.get("volume")
        
        if volume_input:
            val_clean = "".join(filter(str.isdigit, volume_input))
            if val_clean:
                eks.volume = float(val_clean)
                
        if eks.profil_company and eks.komoditas:
            eks.save()
            messages.success(request, f'Data eksportir "{eks.profil_company}" berhasil diperbarui!')
        else:
            messages.error(request, "Profil Company dan Komoditas wajib diisi!")
...
@staff_member_required(login_url='login')
def hapus_eksportir(request, id):
    if request.method != "POST":
        return redirect("kelola_eksportir")
    eks = get_object_or_404(Eksportir, id=id)
    eks.delete()
    messages.success(request, f'Data eksportir "{eks.profil_company}" berhasil dihapus.')
    return redirect("kelola_eksportir")
